...
//p256
void modSqrt(mpz_t, mpz_t, mpz_t);
void p256ToBig(unsigned int *, mpz_t, Curve &cur);
void p256FromBig(unsigned int*, mpz_t , Curve&);
std::vector<unsigned char> compress(point&);
point decompress(std::vector<unsigned char>);
//signature.cpp
void key_from_hex(User& ,Curve& ,const char *);
void generate_key(User& ,Curve& );
void sign_generate(Signature& ,bytes ,User& ,Curve& );
bool sign_verify(Signature& ,bytes ,point& ,Curve& );

//convert.cpp
size_t hex2bin(const char *, size_t , unsigned char *);
void byteAppend(std::vector<unsigned char>& ,std::vector<unsigned char>);
std::vector<unsigned char> byteMarge(std::vector<unsigned char> ,std::vector<unsigned char>);
char* byteToHex(std::vector<unsigned char>);
unsigned int gmpToLong(unsigned long*& ,mpz_t ,unsigned int );
unsigned int gmpToInt(unsigned int*& ,mpz_t ,unsigned int );
std::vector<unsigned char> gmpToByte(mpz_t);
void byteToGmp(mpz_t,std::vector<unsigned char>);
...
struct Curve{//Fp-256 to p256
	...
	Curve(){
		...
		mpz_init_set_str(p_prime,"FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFF",16);
		mpz_init_set_str(a_coeffi,"FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFC",16);
		mpz_init_set_str(b_coeffi,"28E9FA9E9D9F5E344D5A9E4BCF6509A7F39789F515AB8F92DDBCBD414D940E93",16);
		mpz_init_set_str(G.x,"32C4AE2C1F1981195F9904466A39C9948FE30BBFF2660BE1715A4589334C74C7",16);
		mpz_init_set_str(G.y,"BC3736A2F4F6779C59BDCEE36B692153D0A9877CC62A474002DF32E52139F0A0",16);
		mpz_init_set_str(n_ranks,"FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFF7203DF6B21C6052B53BBF40939D54123",16);
	
		mpz_init(lambda);
		mpz_init(temp);
		mpz_init(tempx);
		mpz_init_set_str(u_sqrt,"7ffffffd80000002fffffffe000000017ffffffe800000037ffffffc80000002",16);//+1
